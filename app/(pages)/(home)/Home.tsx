...
export function HomeSection() {
  ...
  return (
    <div className="min-h-[95vh] relative flex-center flex-col text-center overflow-hidden">
      ...
      ></div>
      <DecorativeEllipse classes={`absolute z-[-1] top-0 left-[-7.5rem] h-full w-auto ${styles['oval-gradient']}`} />
      <DecorativeEllipse classes={`absolute z-[-1] top-0 right-[-7.5rem] h-full w-auto scale-x-[-1] ${styles['oval-gradient']}`} />

      <h1 className={`font-bold text-6xl ${styles.title}`}>Construindo o futuro</h1>
      <h2 className={`font-semibold text-2xl w-[35ch] mt-2 mx-auto ${styles.subtitle}`}>
      ...
      <div className="w-full flex-center gap-12 mt-18 py-2">
        ...
        <div className={`flex-center bg-tertiary rounded-2xl shadow-[0_0_15px_0.5px_rgba(0,0,0,0.3)] flex-[0_0_32.5rem] h-[26rem] overflow-hidden ${styles['main-card']}`}>
          <Image
            className=""
            src="/Spaced-Logo.svg"
            alt="Logo"
    ...
    >
      <defs>
        <linearGradient id="ellipseGradient" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="var(--color-primary)" stop-opacity="0" />
          <stop
            offset="100%"
